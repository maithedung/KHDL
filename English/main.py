...
if __name__ == '__main__':
    ...
    for sentence in data:
        ...
        # print(word_list)
        emotion_list.extend(emotion_analysis('./Train/emotions.txt', word_list))
        score = sentiment_analyse(clean_text)
        neg_means.append(score['neg'])
        neu_means.append(score['neu'])
        pos_means.append(score['pos'])
    ...
    # fig.autofmt_xdate()
    # plt.savefig('./Graph/emotion_graph.png')
    # plt.show()

    # Plotting the sentiment on the graph
    labels = [f'{label}' for label in range(1, len(data) + 1)]

    # The width of the bars: can also be len(x) sequence
    width = len(neg_means)/459

    fig, ax = plt.subplots()
    ax.bar(labels, neu_means, width, label='Neu')
    ax.bar(labels, neg_means, width, bottom=neu_means, label='Neg')
    ax.bar(labels, pos_means, width, bottom=neg_means, label='Pos')
    ax.set_ylabel('Scores')
    ax.set_title('Sentiment Graph')
    ax.legend()

    fig.autofmt_xdate()
    plt.savefig('./Graph/sentiment_graph.png') 
    plt.show()
